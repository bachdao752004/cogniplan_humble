...
def get_cell_position_from_coords(coords, map_info, check_negative=True):
    ...
    if check_negative:
        assert sum(cell_position.flatten() >= 0) == cell_position.flatten().shape[0], print(
            cell_position, coords, map_info.map_origin_x, map_info.map_origin_y
        )
    if single_cell:
...
def check_collision(start, end, map_info):
    ...
    x_max = map_info.map_origin_x + (map_info.map.shape[1] - 1) * map_info.cell_size
    y_max = map_info.map_origin_y + (map_info.map.shape[0] - 1) * map_info.cell_size
    if not (x_min <= start[0] <= x_max and y_min <= start[1] <= y_max):
        return True
    if not (x_min <= end[0] <= x_max and y_min <= end[1] <= y_max):
        return True

    start_cell = get_cell_position_from_coords(start, map_info, check_negative=False)
    end_cell = get_cell_position_from_coords(end, map_info, check_negative=False)
    map = map_info.map
