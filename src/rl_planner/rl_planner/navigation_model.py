"""
Navigation PolicyNet - adapted from CogniPlan navigation branch.
Different from exploration PolicyNet: forward() signature takes
(node_inputs, edge_inputs, current_index, node_padding_mask, edge_padding_mask, edge_mask)
instead of exploration's
...
class NavPolicyNet(nn.Module):
    """Navigation PolicyNet from CogniPlan navigation branch.
    
    Key difference from exploration PolicyNet:
    - INPUT_DIM = 9 (includes direction_vector to target)
    - forward() signature: (node_inputs, edge_inputs, current_index, 
                            node_padding_mask, edge_padding_mask, edge_mask)
    """
    def __init__(self, input_dim, embedding_dim):
        super(NavPolicyNet, self).__init__()
