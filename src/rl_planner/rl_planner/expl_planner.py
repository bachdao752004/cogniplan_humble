...
from .exploration_model import PolicyNet
...
class Runner(Node):
    def __init__(self):
        ...
        self.declare_parameter('num_gen_sample', parameter.N_GEN_SAMPLE)
        parameter.N_GEN_SAMPLE = self.get_parameter('num_gen_sample').value
        ...
        self.declare_parameter('frontier_downsample_factor', 1.0)
        ...
        self.declare_parameter('model_subdir', 'expl_model')
        ...
        self.updatebox_pub = self.create_publisher(Marker, '/update_box', 10)
        self.pred_map_pub = self.create_publisher(OccupancyGrid, '/predicted_map', 10)
    ...
    def run(self):
        ...
        if self.robot.node_manager.nodes_dict.find(next_location.tolist()) is not None:
            ...
            self.get_logger().warning("next location not in node dict")
        t4 = time.time()
        ...
        self.run_time_pub.publish(run_time)
        self.waypoint_pub.publish(waypoint_msg)
        self.pred_map_pub.publish(self.pub_predicted_map())
    ...
    def init_agent(self):
        ...
        self.get_logger().info(f"Loading policy model from: {model_file}")
        ...
        if not os.path.exists(generator_file):
            self.get_logger().error(f"Generator file not found: {generator_file}")
        ...
        policy_net.load_state_dict(policy_state['policy_model'])
        ...
        predictor = Evaluator(config, generator, device=self.device, nsample=parameter.N_GEN_SAMPLE)

        self.robot = Agent(policy_net, predictor, self.device, self.publish_graph)

    def pub_predicted_map(self):
        pred = self.robot.pred_mean_map_info.map
        grid_msg = OccupancyGrid()
        grid_msg.header.frame_id = 'map'
        grid_msg.header.stamp = self.get_clock().now().to_msg()
        grid_msg.info.width = pred.shape[1]
        grid_msg.info.height = pred.shape[0]
        grid_msg.info.resolution = self.robot.pred_mean_map_info.cell_size
        grid_msg.info.origin.position.x = self.robot.pred_mean_map_info.map_origin_x
        grid_msg.info.origin.position.y = self.robot.pred_mean_map_info.map_origin_y
        grid_msg.info.origin.orientation.w = 1.0
        pred_int = np.clip(pred, 0, 100).astype(np.int8)
        grid_msg.data = pred_int.flatten().tolist()
        return grid_msg
    ...
    def visualize_graph(self):
        ...
        for coords in self.robot.node_coords:
            node = self.robot.pred_node_manager.nodes_dict.find((coords[0], coords[1]))
            for neighbor_coords in node.data.neighbor_set:
                start = Point()
                start.x = coords[0]
                start.y = coords[1]
        ...
        updatebox = Marker()
        updatebox.header.frame_id = 'map'
        updatebox.header.stamp = self.get_clock().now().to_msg()
        updatebox.type = Marker.LINE_STRIP
        updatebox.scale.x = 0.15
        updatebox.color.r = 0.0
        updatebox.color.g = 0.0
        updatebox.color.b = 1.0
        updatebox.color.a = 0.8
        updatebox.pose.orientation.w = 1.0

        half_size = parameter.UPDATING_MAP_SIZE / 2.0
        corners = [
            Point(x=self.robot_location[0] - half_size, y=self.robot_location[1] - half_size, z=0.0),
            Point(x=self.robot_location[0] + half_size, y=self.robot_location[1] - half_size, z=0.0),
            Point(x=self.robot_location[0] + half_size, y=self.robot_location[1] + half_size, z=0.0),
            Point(x=self.robot_location[0] - half_size, y=self.robot_location[1] + half_size, z=0.0),
            Point(x=self.robot_location[0] - half_size, y=self.robot_location[1] - half_size, z=0.0),
        ]
        updatebox.points = corners
        self.updatebox_pub.publish(updatebox)
