...
N_GEN_SAMPLE = 4  # how many samples do you want to generate (select from: 1, 4, 7)

FREE = 0
OCCUPIED = 100
UNKNOWN = -1

SENSOR_RANGE = 20.0  # meter
UTILITY_RANGE = 0.5 * SENSOR_RANGE  # for each node, frontiers in this range will be considered as utility
MIN_UTILITY = 3.0  # if the number observable frontiers is less than this value, consider it is zero utility
...
THR_TO_WAYPOINT = 1.0 # meter, the waypoint will be considered as arrived if the robot is closer than this value
THR_NEXT_WAYPOINT = 5.0 # meter, the planner will try to plan a waypoint farther than this value
THR_GRAPH_HARD_UPDATE = 8.0 # meter, node and edges in this range will be fully updated

CLUSTER_RANGE = 10.0 # meter, frontiers will be clustered based on this range
